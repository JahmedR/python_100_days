...
def start_timer():
    ...
    short_wrk_brk = SHORT_BREAK_MIN * 60
    long_wrk_brk = LONG_BREAK_MIN * 60

    if reps % 8 == 0:
        count_down(long_wrk_brk)
        t_label.config(text="Break", fg=RED)
    elif reps % 2 == 0:
        count_down(short_wrk_brk)
...
t_label = Label(text="Timer", fg=GREEN, bg=YELLOW, font=(FONT_NAME, 50))
